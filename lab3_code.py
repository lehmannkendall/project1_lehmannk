...
class Dollar(Currency):
    ...
    def __str__(self):
        ...
        if self.value > 1:
            c_name = "Dollars"
        return "{} {}".format(self.value, c_name)
...
class Bank:
    def __init__(self, name, unit, current_account=0):
        self.name = name
        self.unit = unit
        self.current_account = self.unit(current_account)
        if unit == Pound:
    ...
    def deposit(self, c_inst):
        if not type(c_inst) == self.unit:
            return "ERROR: cannot deposit that currency."
        else:
            self.current_account.value = self.current_account.value + c_inst.value
            # or self.current_account.value += 1
